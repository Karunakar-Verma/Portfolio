...
const experiences = [
  ...
  },
  {
    ...
    description:
      "Developed a full-stack MERN application for practical learning.",
    image: utu,
  },
];
    ...
    <div id="about" className="bg-gray-50 py-16">
      ...
      <div className="flex flex-col lg:flex-row max-w-6xl mx-auto gap-12 px-6">
        ...
        <div className="lg:w-1/2 flex flex-col justify-center">
          ...
          >
            I’m{" "}
            <span className="font-semibold text-[#245abf]">
              Karunakar Verma
            </span>
            , a MERN stack developer. I've completed my{" "}
            <span className="font-semibold">
              B.Tech in Computer Science & Engineering from Uttarakhand
              Technical University (2025)
            </span>{" "}
            while building expertise in{" "}
            <span className="font-semibold">
              React.js, Node.js, Express.js, MongoDB
            </span>{" "}
            and modern web technologies. I’m passionate about creating responsive
            web applications and exploring new technologies.
          </p>
        </div>

        {/* Right Section - Auto Sliding Card */}
        <div className="lg:w-1/2 flex justify-center items-start">
          <div className="bg-white shadow-xl rounded-2xl p-6 w-full max-w-md transition-all duration-500">
            <img
              src={exp.image}
              alt={exp.title}
              className="w-28 h-28 md:w-32 md:h-32 mx-auto rounded-full mb-4 object-cover"
            />
            <h3 className="text-xl font-bold text-center mb-1">{exp.title}</h3>
            <p className="text-gray-500 text-center mb-1">{exp.company}</p>
            <p className="text-gray-400 text-center mb-4 text-sm">{exp.date}</p>
            <p className="text-gray-700 text-center px-4">{exp.description}</p>
          </div>
        </div>
      </div>
    </div>
